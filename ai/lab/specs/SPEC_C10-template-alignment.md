...
> **Status:** draft
> **Author:** AI draft (from `ai/analysis/audit-reports/2026-07-06-fullstack-simulation-report.md` findings F2, F6, F7, F9, F10) · **Date:** 2026-07-06 · **Revision:** 1
...
| # | Constraint |
|---|---|
| C1 | Every command/workflow text change is applied to ALL tool-surface copies listed in §5 — the 4 live local copies AND their 4 `templates/` twins (the W-013 "14 copies" lesson: a drifting twin is worse than no fix). |
...
- **tool-surface copies** — the same workflow text exists as
  `.claude/commands/<name>.md`, `.cursor/rules/<name>.mdc`,
  `.github/prompts/<name>.prompt.md`, `.agents/workflows/<name>.md`, each with a
  `templates/` twin. Pitfall: the `.mdc` and `.prompt.md` variants have different
  front-matter — edit the shared body text only.
...
3. In ALL 8 cold-start copies (§5), extend step 2 ("Fill `ai/guide/MODULE_MAP.md`")
...
1. In ALL 8 cold-start copies, insert a new step after the existing step 2:
...
2. In `templates/CLAUDE.md.tmpl` and `templates/AGENTS.md.tmpl`, append to the
   No-Churn rule sentence: "(one exception: the /cold-start pass corrects the
   stamped Build/Test lines after verifying them against real config files)".
...
| workflows (live) | `.claude/commands/cold-start.md`, `.cursor/rules/cold-start.mdc`, `.github/prompts/cold-start.prompt.md`, `.agents/workflows/cold-start.md` | ours | modify (W1.3, W2.1) |
| workflows (twins) | `templates/claude/commands/cold-start.md`, `templates/cursor/rules/cold-start.mdc`, `templates/github/prompts/cold-start.prompt.md`, `templates/agents/workflows/cold-start.md` | ours | modify (W1.3, W2.1) |
...
| T3 | copy parity | for each of the 4 cold-start twin pairs in §5, the body text (after stripping front-matter up to the first blank line) is byte-identical between the live copy and its `templates/` twin. |
