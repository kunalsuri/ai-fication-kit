...
| W-010 | 2026-07-05 | feature | Shazam update mode + `update` command: deterministic updater for already-installed repos — version awareness (`firstInstalled` + `history` in the install manifest), CHANGELOG-parsed what-changed digest, pre/postflight health bracket, obsolete-file detection (deletion behind a real interactive prompt only; `--yes` never unlocks it), declarative rename registry for future template moves, downgrade guard | — (design agreed in-session with the maintainer: `CHANGELOG.md` as the notes source, always-ask deletion, auto-detect + `update` alias, headline-bullet digest) | — | — (review pending: /review-change in a fresh session) | — | branch claude/shazam-update-verification-oq7ahk | `lib/update.mjs` (new), `lib/migrations.mjs` (new), `lib/installer.mjs`, `install.mjs`, `test/run-tests.mjs`, `docs/CLI-REFERENCE.md`, `CHANGELOG.md` | in-review | [inferred] |
