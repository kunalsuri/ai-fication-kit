...
| AUD-R2-11 | MEDIUM | FIXED | `lib/audit.mjs:104-116` | Completing a guided audit never updates the `Last verified: … @ commit` anchor → `drift --git` immediately flags freshly audited rows as stale | `applyEdits` rewrites table rows only; anchor maintenance was never implemented | `audit --git` now offers to move the anchor to HEAD after the human confirms (new `updateAnchorLine`, multi-line notes consumed whole per REVIEW_W-002 finding 1); without `--git` an explicit reminder is printed | `206e24e` + `2b5d0b6` / 2026-07-04 | R2 §11 · R1 #7 |
...
| ID | Sev | Status | Location (at audit time) | Defect (the error) | Root cause | Fix made | Fixed in / date | Trace |
|---|---|---|---|---|---|---|---|---|
| AUD-R3-01 | CRITICAL | FIXED | `.github/workflows/ai-check.yml:27`, `ai/guide/MODULE_MAP.md:7` | `drift . --git --strict` fails on main (8 stale `[verified]` rows) — the repo's own CI gate is red on every push/PR, and `npm run deep-test` fails at step 3 (reproduced 2026-07-04) | PR #23 merged without the re-anchor/re-audit step every prior PR performed; nothing enforces it (see AUD-R2-11) | Human-reviewable re-anchor of the `Last verified` baseline to `2b5d0b6` in the records commit accompanying this PR; recurrence prevented by the AUD-R2-11 fix (`audit --git` now offers the anchor move) | records commit after `2b5d0b6` / 2026-07-04 | R3 §1 |
| AUD-R3-02 | HIGH | FIXED | `lib/installer.mjs:111` + core templates | `--force` child-lock false-positives on the templates' own `[verified]` prose: any edit to a stamped CLAUDE.md/AGENTS.md/MODULE_MAP.md/WORKLOG.md classifies `locked` (reproduced) — the documented `--force` contract is unreachable for exactly those files | Lock tests `diskText.includes("[verified]")`; pristine templates contain that literal in provenance prose | Lock now keys on `[verified]` lines absent from the pristine template (same diff the warning printer used); human-added signatures still lock | `206e24e` / 2026-07-04 | R3 §2 |
| AUD-R3-03 | HIGH | FIXED | `lib/installer.mjs:150-164` vs `:262-275` | Process-2 backups written before any consent; declining the confirm still leaves `CLAUDE_bkp_*.md` behind while printing "Aborted; nothing written." (reproduced) | Backup copy runs at the top of `install()`, before plan display and both confirmations | Backups decided up front, copied only after both confirmations (COPYFILE_EXCL kept); aborted runs leave the repo byte-identical | `206e24e` / 2026-07-04 | R3 §3 |
| AUD-R3-04 | HIGH | FIXED | `install.mjs:228-232` | Re-running `orient` wipes `humanContext` (wizard answers) from `ai/repo-profile.json` (reproduced) | Fresh profile written with no carry-forward; only the install path has one (`installer.mjs:289-295`) | `orient` now carries `humanContext` forward from the profile on disk, same as the installer | `206e24e` / 2026-07-04 | R3 §4 |
| AUD-R3-05 | MEDIUM | FIXED | `install.mjs:50-56` | Trust header "It NEVER overwrites a file you have edited" is false for Process 2 (user-authored CLAUDE.md/AGENTS.md replaced after backup on plain install, no `--force`) | Guarantee prose never updated when Process 2 landed; same class as fixed AUD-R2-15 | Process-2 carve-out sentence added to the trust header | `206e24e` / 2026-07-04 | R3 §5 |
| AUD-R3-06 | MEDIUM | FIXED | `lib/indepth.mjs:42-44` | Gitignore dir rules over-match prefix siblings: `build/` also excludes `builder/` from all indepth metrics (reproduced) | `pattern += "?.*"` makes the trailing slash optional | Dir rules now require the slash before descendants (`(/.*)?$`) | `206e24e` / 2026-07-04 | R3 §6 |
| AUD-R3-07 | MEDIUM | FIXED | `.github/workflows/test.yml:24` | Syntax gate omits `lib/audit.mjs`, `lib/status.mjs`, `lib/doctor.mjs`, `lib/demo.mjs`, `lib/progress.mjs`, `test/run-deep-test.mjs`, `test/release-check.mjs` | Hardcoded list never updated as modules were added | Syntax gate globs `install.mjs lib/*.mjs test/*.mjs` (bash on all three runners) | `206e24e` / 2026-07-04 | R3 §7 |
| AUD-R3-08 | LOW | FIXED | `ai/lab/WORKLOG.md:36`, `templates/ai/lab/WORKLOG.md.tmpl:31` | First ledger row violates the loop it documents: merged with Review `—`, Status stuck `in-review`, Commits cell "(this branch)" unresolvable; template example row reuses ID `W-001` | Row written pre-merge and never updated; example ID collides with the "next W-n" rule | W-001 backfilled (PR #23 ref, recorded review waiver, Status shipped); example row ID now W-000 in template and repo ledger | `206e24e` / 2026-07-04 | R3 §8 |
| AUD-R3-09 | LOW | FIXED | `lib/indepth.mjs:278-291,302-310` | Cargo `[dev-dependencies]` and all Gemfile gems booked as production | Section/group tracking not implemented for those parsers | Cargo section and Gemfile `group :development/:test` tracking; dev deps booked as development | `206e24e` / 2026-07-04 | R3 §9 |
| AUD-R3-10 | LOW | FIXED | `install.mjs:30-31` | Header says `status` writes only with `--json`; it also rewrites `ai/START-HERE.html` whenever the page exists (`status.mjs:119`) | Header not updated when the progress-page refresh landed | Header now states status also refreshes `ai/START-HERE.html` when present | `206e24e` / 2026-07-04 | R3 §10 |
| AUD-R3-11 | LOW | FIXED | `lib/indepth.mjs:10-14` | `runCmd` comment invites space-containing format strings that `cmd.split(" ")` would break; quotes stripped from every token | Convenience comment overstates the parser | `runCmd` takes an argv array (no splitting/quote-stripping); all git call sites converted | `206e24e` / 2026-07-04 | R3 §11 |

### R3 summary

- **11 FIXED / 0 OPEN** — fixed as ledger row W-002 (spec
  `ai/lab/specs/BUGFIX_audit-R3-fixes.md`, review
  `ai/lab/reviews/REVIEW_W-002.md`, verdict approve): fail-first regression
  tests, fix commits `206e24e` + `2b5d0b6`, suite 362/362.
- AUD-R2-11 (R3-01's recurring root cause) closed in the same unit of work;
  the baseline re-anchor to `2b5d0b6` rides the records commit for human
