...
import { KIT_VERSION, isFile, isDir, listWorkspaceMembers, readText, style } from "./util.mjs";

// Helper for running commands. Takes an argv ARRAY and hands it to execFile
// directly (no shell), so every argument — git --format strings included —
// passes through verbatim: no cmd.exe mangling of `%` or `|` on Windows, no
// word-splitting on spaces, no quoting rules at all.
function runCmd(argv, cwd) {
  return new Promise((resolve) => {
    const [bin, ...args] = argv;
    execFile(bin, args, { cwd, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
      if (error) {
  ...
  if (await isFile(gitignorePath)) {
    ...
    if (content) {
      for (let line of content.split(/\r?\n/)) {
        ...
        // A trailing "/" means "this directory and everything under it" — the
        // slash before any descendant is REQUIRED, or `build/` would also match
        // a prefix sibling like `builder/`.
        const isDirRule = line.endsWith("/");
        const body = isDirRule ? line.slice(0, -1) : line;
        let pattern = body.replace(/[-\/\\^$*+?.()|[\]{}]/g, (ch) => {
          if (ch === "*") return ".*";
          if (ch === "?") return ".";
          return "\\" + ch;
        });
        ...
        if (line.startsWith("/")) {
          // The leading "/" was escaped to "\/" above — drop both characters,
          // so the anchor can match repo-relative paths (which never start with /).
...
// PEP 621 pyproject.toml dependency parsing (uv/modern-Poetry style, distinct
// from the [tool.poetry.dependencies] table format handled below): the plain
// `dependencies = [ … ]` array under [project] is production; every quoted
// string inside a [dependency-groups] or [project.optional-dependencies]
// section is development. Non-greedy match from the key to the closing `]`
// — multi-line arrays work, anything the regex misses is simply not counted.
function parsePep621Deps(text) {
  const entries = [];
  const prodMatch = text.match(/(?:^|\n)\s*dependencies\s*=\s*\[([\s\S]*?)\]/);
  if (prodMatch) {
    for (const m of prodMatch[1].matchAll(/["']([^"']+)["']/g)) entries.push({ raw: m[1], category: "prod" });
  }
  const devSectionMatch = text.match(/\[(?:dependency-groups|project\.optional-dependencies)\][\s\S]*?=\s*\[([\s\S]*?)\]/);
  if (devSectionMatch) {
    for (const m of devSectionMatch[1].matchAll(/["']([^"']+)["']/g)) entries.push({ raw: m[1], category: "dev" });
  }
  return entries;
}

// `name` is the string up to the first version/extras separator; `version` is
// whatever remains (or "").
function splitPep621NameVersion(raw) {
  const m = raw.match(/^([^<>=~!\[\s]+)(.*)$/);
  return m ? { name: m[1], version: m[2].trim() } : { name: raw, version: "" };
}

function addPep621Deps(result, text) {
  for (const { raw, category } of parsePep621Deps(text)) {
    const { name, version } = splitPep621NameVersion(raw);
    result.direct++;
    if (category === "dev") result.byCategory.development++; else result.byCategory.production++;
    result.topLevelDeps.push({ name, version, category });
  }
}
...
async function detectDependencies(targetAbs, filesInfo) {
  ...
  if (await isFile(pyprojectPath)) {
    ...
    // PEP 621 style (uv / modern Poetry): [project] dependencies = [ … ] array,
    // distinct from the [tool.poetry.dependencies] table above.
    addPep621Deps(result, txt);
  ...
  if (await isFile(cargoTomlPath)) {
    ...
    // Track WHICH section we are in: [dev-dependencies] entries are development
    // dependencies, not production.
    let cargoSection = null;
    for (let line of txt.split("\n")) {
      line = line.trim();
      if (line.startsWith("[dependencies]")) { cargoSection = "prod"; continue; }
      if (line.startsWith("[dev-dependencies]")) { cargoSection = "dev"; continue; }
      if (line.startsWith("[")) { cargoSection = null; }
      if (cargoSection) {
        const m = line.match(/^([^=\s]+)\s*=\s*/);
        if (m) {
          result.direct++;
          if (cargoSection === "dev") result.byCategory.development++;
          else result.byCategory.production++;
          result.topLevelDeps.push({ name: m[1], version: "*", category: cargoSection });
        }
      }
    }
  ...
  if (await isFile(gemfilePath)) {
    ...
    // Gems inside `group :development` / `group :test` blocks are development
    // dependencies. Heuristic: a group line opens the block, a bare `end` closes it.
    let inDevGroup = false;
    for (let line of txt.split("\n")) {
      line = line.trim();
      if (/^group\b/.test(line)) { inDevGroup = /:(development|test)\b/.test(line); continue; }
      if (line === "end") { inDevGroup = false; continue; }
      const m = line.match(/^gem\s+['"]([^'"]+)['"]/);
      if (m) {
        result.direct++;
        if (inDevGroup) result.byCategory.development++;
        else result.byCategory.production++;
        result.topLevelDeps.push({ name: m[1], version: "*", category: inDevGroup ? "dev" : "prod" });
      }
    }
  ...
  // Workspace members: one level deep, summing each member's own package.json
  // (Node) and pyproject.toml (Python, PEP 621 array style only — matches
  // orient's member scan). transitive/total stay root-lock-derived; members
  // only contribute direct/topLevelDeps.
  for (const member of await listWorkspaceMembers(targetAbs)) {
    const memberPkgPath = path.join(targetAbs, member, "package.json");
    if (await isFile(memberPkgPath)) {
      try {
        const pkg = JSON.parse(await readText(memberPkgPath));
        const deps = pkg.dependencies || {};
        const devDeps = pkg.devDependencies || {};
        const optDeps = pkg.optionalDependencies || {};
        Object.entries(deps).forEach(([name, version]) => {
          result.direct++; result.byCategory.production++;
          result.topLevelDeps.push({ name, version, category: "prod" });
        });
        Object.entries(devDeps).forEach(([name, version]) => {
          result.direct++; result.byCategory.development++;
          result.topLevelDeps.push({ name, version, category: "dev" });
        });
        Object.entries(optDeps).forEach(([name, version]) => {
          result.direct++; result.byCategory.optional++;
          result.topLevelDeps.push({ name, version, category: "optional" });
        });
      } catch { /* malformed */ }
    }
    const memberPyprojectPath = path.join(targetAbs, member, "pyproject.toml");
    if (await isFile(memberPyprojectPath)) {
      addPep621Deps(result, await readText(memberPyprojectPath) || "");
    }
  }
...
async function analyzeCodeStructureAndDocs(targetAbs, filesInfo) {
  ...
  // Module names are directory names — escape regex metacharacters (c++, a+b,
  // foo(bar), …) or new RegExp() throws and aborts the whole analysis.
  const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const modNameRes = new Map(modNames.map((n) => [n, new RegExp(`\\b${escapeRe(n)}\\b`)]));
        ...
        for (const line of content.split("\n")) {
          if (line.includes("import") || line.includes("require") || line.includes("use") || line.includes("using")) {
            for (const other of modNames) {
              if (other !== name && (line.includes(`/${other}/`) || line.includes(`/${other}"`) || line.includes(`/${other}'`) || modNameRes.get(other).test(line))) {
                deps.add(other);
              }
            }
          }
...
async function analyzeGitHistory(targetAbs, warnings) {
  ...
  const tagsCmd = await runCmd(["git", "for-each-ref", "--sort=-taggerdate",
    "--format=%(refname:short)|%(taggerdate:iso8601)|%(subject)", "refs/tags"], targetAbs);
  if (tagsCmd.success) {
...
export async function walk(dir, rootDir, gitignoreRules, filesInfo, warnings) {
  let entries;
  try {
