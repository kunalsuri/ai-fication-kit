...
// Helper for running commands. Runs argv directly (no shell), so callers can
// write git format strings like --format='%(refname:short)' without cmd.exe on
// Windows mangling the `%` placeholders or treating a literal `|` as a pipe.
function runCmd(cmd, cwd) {
  return new Promise((resolve) => {
    const [bin, ...args] = cmd.split(" ").map(part => part.replace(/'/g, ""));
