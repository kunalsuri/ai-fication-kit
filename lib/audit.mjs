...
const defaultAuditIo = { confirm, choose, isInteractive };

export async function audit(targetAbs, flags, io = defaultAuditIo) {
  // Automation must never manufacture a human signature — unlike every other
  // command, --yes does NOT unlock this one.
  if (flags.yes || !io.isInteractive()) {
  ...
  for (const row of rows) {
    ...
    const proceed = await io.confirm("Audit this row now?", flags);
    if (!proceed) { info("  skipped."); continue; }

    const options = [...STABILITY_OPTIONS, "skip (leave this row untouched)"];
    const stability = await io.choose("Set Stability for this row:", options, flags, options.length - 1);
