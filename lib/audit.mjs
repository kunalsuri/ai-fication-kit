...
import {
  backupName, confirm, choose, info, isInteractive, readText, style,
} from "./util.mjs";
...
export async function audit(targetAbs, flags) {
  ...
  const { bkpPath } = await writeAuditedMap(mapPath, mapText, text);
...
export async function writeAuditedMap(mapPath, previousText, newText) {
  const bkpPath = path.join(path.dirname(mapPath), backupName(path.basename(mapPath, ".md"), ".md"));
