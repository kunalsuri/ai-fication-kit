...
import { KIT_VERSION, MANIFEST_REL, PROFILE_REL, backupName, confirm, die, info,
  readText, sha256, templatesRoot } from "./util.mjs";
 ...
 */
export function classifyAction({ diskText, recordedHash, newText, force }) {
  ...
  if (diskText.includes(VERIFIED_TAG)) return "locked";
...
export async function install(targetAbs, profile, flags) {
  ...
  for (const rel of installable) {
    ...
    const action = backedUpFiles.has(destRel)
      ...
      : classifyAction({ diskText, recordedHash: prevHashes[posix(destRel)], newText: out, force: flags.force });
  ...
  for (const k of kept) {
    if (k.action === "locked") {
      info(`  keep (child-lock: human ${VERIFIED_TAG} content — never overwritten)  ${k.destRel}`);
    } else {
  ...
  // OSes and between the Node and Python installers. fileHashes records the
