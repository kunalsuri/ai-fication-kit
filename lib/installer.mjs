...
import { DETECTORS } from "./orient.mjs";
...
// resolved from orient's own marker table (DETECTORS) so this never drifts
// from what orient itself recognizes. Each hit is backticked so `verify`
// treats it as a path claim; the whole point is that claim must always
// resolve. DETECTORS markers are plain filenames (no globs), so a plain
// exists() check per marker is sufficient — the *.sln/*.csproj glob and bare
// Makefile fallback detectors in orient() are deliberately not duplicated
// here (out of scope; a C#/.NET- or Makefile-only repo falls back to the
// plain-text phrase below, which is safe — no broken claim — if slightly
// less specific). The Set dedup guards against a future duplicate marker
// landing in DETECTORS; harmless today since all markers are already unique.
...
async function placeholders(profile, targetAbs) {
...
export function destinationFor(rel) {
  ...
  if (dest.endsWith(".lite.md.tmpl")) {
    dest = dest.slice(0, -".lite.md.tmpl".length) + ".md";
  } else if (dest.endsWith(".tmpl")) {
    dest = dest.slice(0, -".tmpl".length);
  }
  return dest;
}

/** @internal exported for testing — the install manifest is untrusted input
 * (a repo's ai/install-manifest.json can be edited, corrupted, or planted), so
 * every path read from it must be a clean posix-relative path before any later
 * pass (obsolete-file scan, hash provenance, uninstall) acts on it: no NUL, no
 * backslashes, not absolute, no drive letter, and no "", "." or ".." segments. */
...
export async function install(targetAbs, profile, flags) {
  ...
  const vars = await placeholders(profile, targetAbs);
  // templates/README.md documents the templates themselves and is not installed.
  const allTemplates = (await listTemplateFiles()).filter(rel => rel !== "README.md");
  let installable;
  if (flags.lite) {
    installable = allTemplates.filter(rel => {
      const p = rel.split(path.sep).join("/");
      if (p.startsWith("ai/")) return false;
      if (p === "github/workflows/ai-check.yml.tmpl") return false;
      if (p === "CLAUDE.md.tmpl" || p === "AGENTS.md.tmpl") return false;
      return true;
    });
  } else {
    installable = allTemplates.filter(rel => {
      const p = rel.split(path.sep).join("/");
      if (p === "CLAUDE.lite.md.tmpl" || p === "AGENTS.lite.md.tmpl") return false;
      return true;
    });
  }
  ...
  const manifestRel = flags.lite ? path.join(".agents", "install-manifest.json") : MANIFEST_REL;
  const prevText = (await readText(path.join(targetAbs, manifestRel))) ||
                   (!flags.lite ? await readText(path.join(targetAbs, ".agents", "install-manifest.json")) : null);
  let prevFiles = [];
  let prevHashes = {};
  ...
  if (prevText) {
    try {
      ...
      // Unsafe path entries are dropped on load — see isSafeManifestPath.
      if (parsed && Array.isArray(parsed.files)) {
        prevFiles = parsed.files.filter(isSafeManifestPath);
      }
      if (parsed && parsed.fileHashes && typeof parsed.fileHashes === "object") {
        prevHashes = Object.fromEntries(
          Object.entries(parsed.fileHashes).filter(([k]) => isSafeManifestPath(k)));
      }
  ...
  const nonTemplateOwned = new Set([
    posix(PROFILE_REL), posix(MANIFEST_REL), ".agents/install-manifest.json", "ai/repo-indepth.json", PROGRESS_PAGE_DEST,
  ]);
  const obsolete = [];
  ...
  if (!flags.lite) info(`  write      ${PROFILE_REL}   (the orient profile)`);
  info(`  write      ${posix(manifestRel)}  (for clean uninstall + re-run provenance)`);
  ...
  const totalWriteCount = flags.lite ? plan.length + 1 : plan.length + 2;
  if (!(await confirm(`Write ${totalWriteCount} file(s) into ${targetAbs}?`, flags))) {
  ...
  const extraFiles = flags.lite
    ? [posix(manifestRel)]
    : [posix(PROFILE_REL), posix(MANIFEST_REL)];
  if (!flags.lite && await exists(path.join(targetAbs, indepthRel))) extraFiles.push(posix(indepthRel));
  ...
  await fs.mkdir(path.dirname(path.join(targetAbs, manifestRel)), { recursive: true });
  await fs.writeFile(path.join(targetAbs, manifestRel),
    JSON.stringify(manifest, null, 2) + "\n", "utf8");

  info(`\n✓ Installed ${totalWriteCount} file(s)${flags.lite ? " (lite mode — no ai/ directory created)" : ""}.`);
  ...
  if (!flags.lite) await refreshProgressPage(targetAbs);
  return true;
}

export async function uninstall(targetAbs, flags) {
  let manifestRel = MANIFEST_REL;
  let manifestText = await readText(path.join(targetAbs, manifestRel));
  if (!manifestText) {
    const liteRel = path.join(".agents", "install-manifest.json");
    const liteText = await readText(path.join(targetAbs, liteRel));
    if (liteText) {
      manifestRel = liteRel;
      manifestText = liteText;
    }
  }
  if (!manifestText) die(`No install-manifest.json found in ${targetAbs} — nothing to uninstall.`);
  let manifest;
  try {
    manifest = JSON.parse(manifestText);
  } catch {
    die(`Could not parse ${manifestRel}.`);
  }
