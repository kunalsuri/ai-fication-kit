...
import { KIT_FOOTER_MARKER, KIT_VERSION, MANIFEST_REL, PROFILE_REL, ask, confirm, die, exists,
  info, readText, reserveBackupPath, sha256, style, templatesRoot } from "./util.mjs";
...
export async function install(targetAbs, profile, flags) {
  ...
  for (const [srcFile, base] of [["CLAUDE.md", "CLAUDE"], ["AGENTS.md", "AGENTS"]]) {
    ...
    if (diskNow !== null && !diskNow.includes(KIT_FOOTER_MARKER) && !diskNow.includes(VERIFIED_TAG)) {
      ...
      if (!flags.dryRun) {
        await fs.copyFile(srcAbs, bkpAbs, fsConstants.COPYFILE_EXCL);
        info(`  ℹ Backed up ${srcFile} → ${bkpRel} (knowledge preserved for /cold-start)`);
      } else {
        info(`  ℹ Would back up ${srcFile} → ${bkpRel} (knowledge preserved for /cold-start)`);
      }
  ...
  const manifest = {
    ...
    files: [...new Set([...prevFiles, ...plan.map(p => posix(p.destRel)), ...extraFiles])].sort(),
