...
import { KIT_VERSION, ask, choose, confirm, info, isDir, isInteractive, readText, style } from "./util.mjs";
...
export const PRIMARY_TOOL_OPTIONS = [
  "Claude Code",
  "GitHub Copilot",
  "Cursor",
  "Google Antigravity",
...
// Read-only, best-effort default for the "which AI tool?" question below.
// Home-dir/target inspection only — never a write, and any read failure
// (permissions, sandboxed home dir, etc.) is silently tolerated: worst case
// we fall back to asking with no default guess.
// Plain-text (no color codes) step-1 instruction for the post-install "next
// steps" printer, tailored to the wizard's `primaryTool` answer. Falls back
// to the Claude Code instruction for "Several of these", an unrecognized
// value, or when no wizard ran at all (automation / --yes / no TTY) — that
// fallback is deliberate: it is today's unchanged output.
const COLD_START_INSTRUCTIONS = {
  "Claude Code": "Open the repo in Claude Code and run /cold-start",
  "GitHub Copilot": "Open the repo in VS Code and type /cold-start in Copilot Chat",
  Cursor: "Open the repo in Cursor and invoke the cold-start rule (.cursor/rules/cold-start.mdc)",
  "Google Antigravity": "Open the repo in Antigravity and run the cold-start workflow in the Agent Manager",
};
export function coldStartInstructionFor(primaryTool) {
  return COLD_START_INSTRUCTIONS[primaryTool] || COLD_START_INSTRUCTIONS["Claude Code"];
}

export async function detectPrimaryTool(targetAbs, homeDir = os.homedir()) {
  if (await isDir(path.join(homeDir, ".claude"))) return "Claude Code";
  if (await isDir(path.join(targetAbs, ".cursor")) || await isDir(path.join(homeDir, ".cursor"))) return "Cursor";
  try {
    const extDir = path.join(homeDir, ".vscode", "extensions");
    const entries = await fs.readdir(extDir);
    if (entries.some((n) => n.toLowerCase().startsWith("github.copilot"))) return "GitHub Copilot";
  } catch { /* no ~/.vscode/extensions, or unreadable — tolerate silently */ }
  return null;
}
...
// Real interactive I/O, overridable so tests can script deterministic answers
// and force isInteractive() true without allocating a real TTY.
const defaultIo = { ask, choose, confirm, isInteractive };

// Returns a humanContext object, or null when the wizard is skipped (CI/--yes/no TTY).
// May exit(0) if the user chooses to stop and set up version control or a branch first.
export async function runFirstRunWizard(targetAbs, profile, flags, io = defaultIo) {
  if (flags.yes || !io.isInteractive()) return null;
  ...
  const skillLevel = await io.choose("How would you describe your development experience?", SKILL_LEVELS, flags, 1);
  const codebaseFamiliarity = await io.choose("How well do you know THIS codebase?", FAMILIARITY, flags, 0);
  ...
  if (!branch.versionControlled) {
    ...
    acknowledgedRisk = await io.confirm("Proceed without version control?", flags);
    if (!acknowledgedRisk) {
    ...
    if (isDefaultBranch) {
      ...
      acknowledgedRisk = await io.confirm(`Continue on '${branch.name}' anyway?`, flags);
      if (!acknowledgedRisk) {
  ...
  if (profile.maturity?.process === 2) {
    ...
    const proceed = await io.confirm("Proceed with backup and install?", flags);
    if (!proceed) {
  ...
  const shape = await io.choose("How is your codebase structured?", [
  ...
  if (shape.startsWith("That's right")) {
    ...
    const description = await io.ask("What is the stack? (e.g. 'Go', 'Python + Django')", flags, detected);
    stack = { kind: "single", detected: profile.languages, description };
  } else if (shape.startsWith("Split")) {
    const frontend = await io.ask("Frontend stack? (e.g. 'React + TypeScript')", flags, "");
    const backend = await io.ask("Backend stack? (e.g. 'Java / Spring', 'Python / FastAPI')", flags, "");
  ...
  // 4 — Primary AI tool: tailors the post-install "next steps" message. All
  // tools' templates are still installed regardless of the answer — only the
  // messaging changes.
  const detectedTool = await detectPrimaryTool(targetAbs);
  const defaultToolIndex = detectedTool
    ? PRIMARY_TOOL_OPTIONS.indexOf(detectedTool)
    : PRIMARY_TOOL_OPTIONS.length - 1; // "None yet" — no evidence, no guess
  const primaryTool = await io.choose("Which AI coding tool will you use?", PRIMARY_TOOL_OPTIONS, flags, defaultToolIndex);
