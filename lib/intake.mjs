...
import { KIT_VERSION, ask, choose, confirm, info, isInteractive, readText } from "./util.mjs";
...
export async function runFirstRunWizard(targetAbs, profile, flags) {
  ...
  if (!branch.versionControlled) {
    ...
    info("    before letting any agent edit code — it is your undo button.");
    ...
    if (isDefaultBranch) {
      info("⚠️  That looks like your production/default branch. Setup and the agent edits");
      info("    that follow can disturb live code. Best practice is a throwaway branch:");
      info("       git checkout -b ai-fication-setup");
