...
// whose extension is in the filename allow-list below. `node.js`/`vue.js`
// (ext `js`) and `console.log`/`this.state` all need this explicit deny-list.
...
// A bare `name.ext` token (no slash) is only a filename CLAIM when `ext` is a
// real file extension. Without this closed allow-list, prose API/method
// references that share the `name.ext` shape — `util.inspect`, `array.map`,
// `foo.bar` — were mis-read as missing files and failed `verify --strict`. Path
// claims (anything containing `/`) are unaffected and always checked.
const KNOWN_FILE_EXTS = new Set([
  // JS/TS + web
  "js", "mjs", "cjs", "jsx", "ts", "tsx", "vue", "svelte", "css", "scss", "sass",
  "less", "html", "htm",
  // other languages
  "py", "pyi", "rb", "go", "rs", "java", "kt", "kts", "scala", "clj", "php",
  "swift", "c", "cc", "cpp", "cxx", "h", "hpp", "hxx", "cs", "m", "mm", "dart",
  "lua", "r", "pl", "sh", "bash", "zsh", "ps1", "bat", "sql", "ex", "exs", "jl",
  // data / config
  "json", "yml", "yaml", "toml", "ini", "cfg", "conf", "xml", "csv", "tsv", "env",
  "lock", "properties", "gradle", "proto", "graphql", "tf", "hcl", "prisma",
  "mod", "sln", "csproj", "fsproj", "vbproj",
  // docs / markup / assets
  "md", "mdx", "markdown", "rst", "adoc", "txt", "pdf", "svg", "png", "jpg",
  "jpeg", "gif", "webp", "ico",
  // this kit's own doc/template extensions
  "mmd", "cff", "tmpl", "mdc",
]);
...
function extractClaims(text, sourceFile) {
  ...
  for (let i = 0; i < lines.length; i++) {
    // Forward-looking / illustrative regions opt out: paths named inside them are
    // examples or not-yet-created files, not current-state claims. Used by the
    // roadmap's Planned detailed specs. Markers are on their own comment line.
    if (/<!--\s*verify-ignore:start\s*-->/.test(lines[i])) { ignoring = true; continue; }
    if (/<!--\s*verify-ignore:end\s*-->/.test(lines[i])) { ignoring = false; continue; }
    ...
    for (const m of lines[i].matchAll(/`([^`\n]+)`/g)) {
      ...
      // npm scoped package (`@scope/pkg`), not a path.
      if (/^@[\w.-]+\//.test(s)) continue;
      s = s.replace(/\\/g, "/").replace(/^\.\//, "")
        .replace(/::[^\s`]*$/, "") // pytest selector (`file.py::test_name`) — strip before the `:N` line-suffix strip
        .replace(/:\d+(-\d+)?$/, "");
      ...
      if (isDirClaim || s.includes("/")) type = "path";
               ...
               KNOWN_FILE_EXTS.has(s.slice(s.lastIndexOf(".") + 1).toLowerCase()) &&
               !VERIFY_NON_FILES.has(s.toLowerCase())) type = "filename";
      else continue; // bare words, API refs (util.inspect), tags, tool names, commands
...
// Fallback existence probe for a claim the pruned index missed. The claim text
// comes from the scanned docs, so it is untrusted: resolve it and refuse
// anything that lands outside the target (traversal segments like a/../../b,
// or an absolute path) — this command must never stat outside the repo tree.
// Same containment rule as uninstall's path check in lib/installer.mjs.
async function probeInsideTarget(targetAbs, lookup) {
  const abs = path.resolve(targetAbs, lookup);
  const rel = path.relative(path.normalize(targetAbs), abs);
  if (rel === "" || rel.startsWith("..") || path.isAbsolute(rel)) return false;
  return (await isFile(abs)) || (await isDir(abs));
}
...
// Pure compute: the claim-extraction + file-existence scan behind `verify`,
// with no console output and no writes. Returns null if there are no
// knowledge docs to check. Shared with `status`.
export async function computeVerification(targetAbs) {
  ...
  // The work ledger: every row links its spec/review/eval by backticked path,
  // so a row whose artifacts vanished must fail here instead of rotting silently.
  if (await isFile(path.join(targetAbs, "ai", "lab", "WORKLOG.md"))) {
    sources.push("ai/lab/WORKLOG.md");
  }
  // The feature roadmap: Shipped rows link each feature's spec, and Planned rows
  // link a spec once drafted — same trace discipline as the ledger. Forward-looking
  // detail (future/illustrative paths) is fenced with `verify-ignore` markers.
  if (await isFile(path.join(targetAbs, "ai", "lab", "ROADMAP.md"))) {
    sources.push("ai/lab/ROADMAP.md");
  }
  ...
  for (const c of claims) {
    if (c.type === "path") {
      ...
      } else if (await probeInsideTarget(targetAbs, c.lookup)) {
        // The index prunes build-output names (bin/, out/, target/, …) that are
        // real source directories in some repos — a direct disk probe keeps a
        // truthful claim from being reported missing.
        c.status = "confirmed"; c.foundAt = c.lookup; confirmed++;
      } else {
  ...
  return { sources, claims, confirmed, moved, missing };
}

export async function verify(targetAbs, flags) {
  const result = await computeVerification(targetAbs);
  if (!result) {
    die("Nothing to verify: no CLAUDE.md/AGENTS.md/ai/ docs found. Run install (or shazam) first.");
  }
  const { sources, claims, confirmed, moved, missing } = result;
  ...
  if (!flags.dryRun) {
    const summary = [`## ${bad.length ? "❌" : "✅"} ai-fication-kit \`verify\``, ""];
    if (bad.length) {
      for (const c of bad) {
        const fix = c.status === "missing"
          ? `it no longer exists`
          : `it looks like it moved to \`${c.foundAt}\``;
        summary.push(`- Your docs mention \`${c.claim}\` (${c.sourceFile} line ${c.line}), but ${fix} — ` +
          `edit ${c.sourceFile} line ${c.line}, or ask your agent to run /check-drift.`);
      }
    } else {
      summary.push(`All ${confirmed} claim(s) confirmed.`);
    }
