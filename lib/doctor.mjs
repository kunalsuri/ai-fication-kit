...
export async function diagnose(targetAbs) {
  ...
  if (inferredRows.length) {
    return {
      ...
      action: `Do a human audit: run node install.mjs audit "${targetAbs}" (see docs/AUDIT-GUIDE.md).`,
    };
  }
  ...
  if (verifyBad || driftBad) {
    ...
    return {
      ...
      action: `node install.mjs verify "${targetAbs}" --strict && node install.mjs drift "${targetAbs}"${gitFlag} --strict`,
    };
  }
