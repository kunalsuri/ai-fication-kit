...
// read, and reduces it all to one verdict line. No writes of its own unless
// --json (STATUS.json); it does still refresh ai/START-HERE.html when that
// living progress page already exists (see lib/progress.mjs).
...
function newestAuditDate(mapText) {
  let newest = null;
  // Accept both stamp formats: DD/MM/YYYY (what the `audit` command writes) and
  // ISO YYYY-MM-DD (what docs/AUDIT-GUIDE.md teaches for hand audits).
  for (const m of mapText.matchAll(/\[verified\]\s*\((?:(\d{2})\/(\d{2})\/(\d{4})|(\d{4})-(\d{2})-(\d{2}))/g)) {
    const d = m[1] !== undefined
      ? new Date(Date.UTC(Number(m[3]), Number(m[2]) - 1, Number(m[1])))
      : new Date(Date.UTC(Number(m[4]), Number(m[5]) - 1, Number(m[6])));
...
export async function computeStatus(targetAbs) {
  ...
  // Doctor already knows the user's workflow stage (1-5, purely from files on
  // disk). Stages 1-2 short-circuit the DRIFTING/NEEDS AUDIT/TRUSTED logic
  // below — a brand-new repo hasn't earned a scary verdict yet.
  ...
  if (doctor.step === 1) {
    ...
    verdict = "NOT MAPPED YET";
  } else if (brokenClaims > 0 || driftItems > 0) {
    verdict = "DRIFTING";
  } else if (mapText === null ||
    rows.verified + rows.inferred + rows.unknown === 0 || // scaffolded map: zero rows is not trust
    rows.inferred > 0 || rows.unknown > 0 ||
  ...
  const badgeColor = verdict === "TRUSTED" ? "brightgreen"
    : verdict === "NEEDS AUDIT" ? "yellow"
    : verdict === "NOT MAPPED YET" ? "blue"
    : verdict === "NOT INSTALLED" ? "grey"
    : "red"; // DRIFTING
...
export function printStatusReport(result) {
  const verdictColor = result.verdict === "TRUSTED" ? style.green
    : result.verdict === "NEEDS AUDIT" ? style.amber
    : result.verdict === "NOT MAPPED YET" ? style.blue
    : result.verdict === "NOT INSTALLED" ? style.dim
    : style.red; // DRIFTING
  info("");
  info(style.bold("ai-fication-kit status"));
  info("");
  if (result.verdict === "NOT MAPPED YET") {
    info("  expected at this stage — the map hasn't been drafted yet");
  }
  ...
  info(`  Broken claims:     ${result.brokenClaims}` + (result.verification ? "" : " (no knowledge docs to check)"));
  ...
  if (result.verdict === "NOT INSTALLED" || result.verdict === "NOT MAPPED YET") {
    info("  Next: " + result.doctorAction);
  }
  info("");
}
