...
export async function refreshProgressPage(targetAbs) {
  ...
  // <-escape so a "</script>" inside any value (e.g. an absolute path in
  // doctorAction) cannot close the JSON block and inject markup into the page.
  const json = JSON.stringify(data).replace(/</g, "\\u003c");
  const updated = existing.replace(DATA_BLOCK_RE, (_m, open, close) => open + json + close);
