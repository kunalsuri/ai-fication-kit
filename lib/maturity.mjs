...
import { KIT_VERSION, KIT_FOOTER_MARKER, info as rawInfo, isDir, isFile, listWorkspaceMembers, readText, style } from "./util.mjs";

// Paint the box borders, ✓/✗ marks, and the score bar — only on a color TTY. It runs
// on the fully-built line, so it never disturbs the box's fixed-width padding maths.
// When color is off (pipes, CI, the test harness) it returns the string untouched.
function paint(s) {
  if (!style.enabled) return String(s);
  return String(s)
    .replace(/[╔╗╚╝═╠╣║]/g, (ch) => style.coral(ch))
    .replace(/✓/g, style.green("✓"))
    .replace(/✗/g, style.red("✗"))
    .replace(/△/g, style.amber("△"))
    .replace(/[█▓▒░]/g, (ch) => style.coral(ch));
}
const info = (s) => rawInfo(paint(s));
...
async function checkDependencyLocks(targetAbs) {
  const locks = [
    ...
    "poetry.lock", "Pipfile.lock", "uv.lock", "Gemfile.lock", "composer.lock",
    "Cargo.lock", "go.sum",
  ];
  const found = [];
...
async function checkCodeStructure(targetAbs) {
  ...
  // One level deep into workspace members — a split frontend/backend repo
  // often has no top-level src/, only member/src/.
  for (const member of await listWorkspaceMembers(targetAbs)) {
    for (const d of codeDirs) {
      if (await isDir(path.join(targetAbs, member, d))) found.push(member + "/" + d + "/");
    }
  }
  // Monorepo markers
  const monorepo = [];
...
export async function checkMaturity(targetAbs) {
  ...
  const aiLayerInstalled = await isFile(path.join(targetAbs, "ai", "repo-profile.json"));
...
export function printMaturityReport(result) {
  ...
  // One source of truth for the box width. Borders and every row derive from INNER,
  // so nothing can drift out of alignment. clip() pads short rows and trims long ones
  // to exactly INNER visible columns; paint() (above) adds color afterwards, which is
  // zero-width and therefore never affects this maths.
  const INNER = 58;
  const top = "╔" + "═".repeat(INNER) + "╗";
  const sep = "╠" + "═".repeat(INNER) + "╣";
  const bot = "╚" + "═".repeat(INNER) + "╝";
  const clip = (s) => (s.length > INNER ? s.slice(0, INNER) : s + " ".repeat(INNER - s.length));
  const row = (s) => info("║" + clip(s) + "║");
  const center = (s) => {
    const pad = Math.max(0, INNER - s.length);
    const left = Math.floor(pad / 2);
    return " ".repeat(left) + s + " ".repeat(pad - left);
  };
  // A checked line: "  <mark> <label padded to 16> <value>", trimmed to the box.
  const field = (mark, label, value) => row(`  ${mark} ${label.padEnd(16)} ${value}`);
  const bar = (score) => {
  ...
  const processLabel = result.process === 1
    ? "Legacy (Process 1) — create ai/ fresh"
    : "Modern (Process 2) — backup, then ai/";

  info("");
  info(top);
  row(center("Repository Maturity Report"));
  row(center(`${path.basename(result.target)}  ·  ${result.generated.slice(0, 10)}`));
  info(sep);
  row(`  Overall Score:  ${bar(result.score)}  ${String(result.score).padStart(3)}/100`);
  row(`  AI Readiness:   ${processLabel}`);
  info(sep);
  ...
  if (aiLabel.length) {
    field("✓", "AI Config", aiLabel[0]);
    for (let i = 1; i < aiLabel.length; i++) field(" ", "", aiLabel[i]);
  } else {
    field("✗", "AI Config", "None detected");
  }

  // Version control
  if (c.versionControl.exists) {
    field("✓", "Version Control", `git, branch: ${c.versionControl.branch || "?"}`);
  } else {
    field("✗", "Version Control", "Not a git repository");
  }

  // Build system
  if (c.buildSystem.exists) field("✓", "Build System", c.buildSystem.markers.slice(0, 3).join(", "));
  else field("✗", "Build System", "No markers detected");
  ...
  if (c.testInfra.exists) {
    ...
    field("✓", "Test Infra", parts.join("; "));
  } else {
    field("✗", "Test Infra", "None detected");
  }

  // CI/CD
  if (c.cicd.exists) field("✓", "CI/CD", c.cicd.systems.join(", "));
  else field("✗", "CI/CD", "None detected");
  ...
  if (c.documentation.readmeExists) {
    ...
    field("✓", "Documentation", `README (${size})`);
  } else {
    field("✗", "Documentation", "No README found");
  }

  // Dependencies
  if (c.dependencyLocks.exists) field("✓", "Dependencies", c.dependencyLocks.files.join(", "));
  else field("✗", "Dependencies", "No lock files");

  // Code structure
  if (c.codeStructure.dirs.length) field("✓", "Code Structure", c.codeStructure.dirs.join(", "));
  else field("✗", "Code Structure", "No standard dirs (src/, lib/, ...)");

  // License
  if (c.license.exists) field("✓", "License", c.license.file);
  else field("✗", "License", "None found");

  // Gitignore
  if (c.gitignore.exists) {
    const note = c.gitignore.coversCommon
      ? "(comprehensive)"
      : `(missing: ${c.gitignore.missing.slice(0, 3).join(", ")})`;
    field(c.gitignore.coversCommon ? "✓" : "△", ".gitignore", note);
  } else {
    field("✗", ".gitignore", "Not found");
  }

  info(sep);

  // Process summary
  if (result.aiLayerInstalled) {
    row("  → ai/ layer already installed — next: /cold-start (if the map is empty) or the human audit");
  } else if (result.process === 2) {
    row("  → Process 2 will run:");
    if (c.aiConfig.claudeMd.exists && !c.aiConfig.claudeMd.hasKitFooter) {
      row("    1. Back up CLAUDE.md → CLAUDE_bkp_<timestamp>.md");
    }
    if (c.aiConfig.agentsMd.exists && !c.aiConfig.agentsMd.hasKitFooter) {
      row("    2. Back up AGENTS.md → AGENTS_bkp_<timestamp>.md");
    }
    row("    3. Create ai/ knowledge layer from templates");
    row("    4. /cold-start will extract knowledge from backups");
  } else {
    row("  → Process 1 will run:");
    row("    1. Create ai/ knowledge layer from templates");
    row("    2. Create CLAUDE.md + AGENTS.md from templates");
    row("    3. /cold-start drafts ai/guide/ docs from code");
  }
