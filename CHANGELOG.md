...
- **Living progress page (`ai/START-HERE.html`)** — a fully offline dashboard
  ...
  and a small glossary. Regenerated by `lib/progress.mjs`'s
  `refreshProgressPage()` at the end of `install`, `verify`, `drift`,
  `status`, and `audit` — zero external requests, works by double-click.
  `uninstall` removes it like any other stamped file; `verify`/`drift`/
  `status` still work fine if it's ever deleted. `lib/installer.mjs` exempts
  its destination path from the usual edited-file/child-lock detection,
  since its content legitimately changes on every run.
