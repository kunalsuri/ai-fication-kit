...
**Implement → Review → Evaluate → Record** half of the
[engineering loop](METHODOLOGY.md#7-the-engineering-loop--the-steady-state-after-the-map-is-trusted),
taking a finished spec from `ai/lab/specs/` as its single source of truth. It is
...
| **4. Verify** | The spec's *entire* test plan implemented; suites green with real output pasted; acceptance criteria scored one by one. | Evaluate |
