...
| **Cursor / Codex** | (no dedicated tree yet) | manual: paste command bodies as prompts |
...
| Command | Claude Code | Copilot Chat | Antigravity |
|---|---|---|---|
| `cold-start` — draft the maps | `/cold-start` | `/cold-start` (prompt) | `cold-start` workflow |
| `add-feature` — safeguarded development | `/add-feature` | `/add-feature` | `add-feature` workflow |
| `check-drift` — drift diagnostics | `/check-drift` | `/check-drift` | `check-drift` workflow |
| `create-feature-catalog` — mine implemented features | `/create-feature-catalog` | `/create-feature-catalog` | workflow |
| `post-cold-start-verification` — semantic gap audit | `/post-cold-start-verification` | `/post-cold-start-verification` | workflow |
| `review-agent-config` — config consistency gate | `/review-agent-config` | `/review-agent-config` | workflow |
| `verify-ai-readiness` — maturity rating | `/verify-ai-readiness` | `/verify-ai-readiness` | workflow |
| `perform-feature-add-simulation` — dry-run friction check | `/perform-feature-add-simulation` | `/perform-feature-add-simulation` | workflow |
...
## Cursor / Codex (manual mode)

No dedicated template tree yet — both tools read `AGENTS.md` for the rules and
the `ai/` maps for knowledge, but the commands are driven by hand:
...
- Full command semantics (what each workflow does): the command files themselves
  ...
  content across the three template trees.
